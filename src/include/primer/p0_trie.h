...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    ...
    return &(children_[key_char] = std::forward<std::unique_ptr<TrieNode>>(child));
  }
  ...
  std::unique_ptr<TrieNode> *GetChildNode(char key_char) {
    auto temp = children_.find(key_char);
    if (temp == children_.end()) {
      return nullptr;
    }
    return &(temp->second);
  }
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (size_t i = 0; i + 1 < key.length(); i++) {
      if (!now->get()->HasChild(key[i])) {
        now = now->get()->InsertChildNode(key[i], std::make_unique<TrieNode>(key[i]));
      } else {
